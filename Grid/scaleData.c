		...
		for( jl=0; jl<nyl; jl++ ) {
			for( il=0; il<nxl-1; il++ ) {
                ...
                for( i2b=0; i2b < blowup; i2b++ ) {
                    ...
                    if( il*blowup+i2b+offset_xb + jl*blowup*nxb + offset_yb*nxb >= array_size ) { printf("expand_data: mem error 003\n" ); exit(-1); }
        ...
        for( jl=0; jl<nyl-1; jl++ ) 
            for( il=0; il<nxl;   il++ ) {
                ...
                for( j2b=0; j2b < blowup; j2b++ ) {
                    ...
                    if( il*blowup+offset_xb + jl*blowup*nxb + (j2b+offset_yb)*nxb >= array_size ) { printf("expand_data: mem error 005\n" ); exit(-1); }
